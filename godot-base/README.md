...
- **Triangle Rush options** under **Settings → Game** tune the base game
  itself: triangle size (60–160%, 150% by default), triangle speed (50–200%,
  50% by default so targets read clearly), the final-stretch speed rush added as
  the timer runs out (0–100%) and the round length (15–90 seconds). They also
  apply from the next round, stack with the Handicap assists (size and speed
  multiply, extra time is added on top) and can make the game harder as well as
  easier.
...
runtime; those ranges live in `games/target_rush/target_rush_options.gd` and are
published to `Settings` through the manifest's `tunables`. Keyboard bindings are
...
4. Optional extras, all declared on the manifest:
   ...
   - `tunables` — player-facing numeric options, stored and clamped by
     `Settings` and read back with `Settings.tunable(key)`. Use a globally
     unique `section/name` key, and add a matching row to
     `settings_menu.tscn` if you want it on the *Game* tab.
